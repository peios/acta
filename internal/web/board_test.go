...
func TestDeleteStatusBlockedWhenNonEmpty(t *testing.T) {
	...
	todo := statusID(t, client, base, "To do")
	postJSON(t, client, base+"/general/items", token, map[string]any{
		"status_id": todo, "title": "Blocker",
	}).Body.Close()

	resp := postJSON(t, client, base+"/general/statuses/"+todo+"/delete", token, nil)
	defer resp.Body.Close()
...
func TestCreateAndDeleteStatus(t *testing.T) {
	...
	resp := postJSON(t, client, base+"/general/statuses", token, map[string]any{"name": "Blocked"})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("create status: want 200, got %d", resp.StatusCode)
	}
	id := decodeID(t, resp)
	if !strings.Contains(getBody(t, client, base+"/general", http.StatusOK), `value="Blocked"`) {
		t.Fatal("new status not on the board")
	}

	// Empty lane deletes cleanly.
	del := postJSON(t, client, base+"/general/statuses/"+id+"/delete", token, nil)
	del.Body.Close()
...
func TestBoardRejectsMissingCSRF(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, base+"/general/items", bytes.NewReader(b))
	req.Header.Set("Content-Type", "application/json")
	resp, err := client.Do(req)
	if err != nil {
		t.Fatal(err)
...
func TestStatusColorSetValidatedAndRendered(t *testing.T) {
	...
	resp := postJSON(t, client, base+"/general/statuses/"+todo+"/color", token, map[string]any{"color": want})
	resp.Body.Close()
	if resp.StatusCode != http.StatusNoContent {
	...
	bad := postJSON(t, client, base+"/general/statuses/"+todo+"/color", token, map[string]any{"color": "#ff0000"})
...
func TestCreateStatusReturnsColor(t *testing.T) {
	...
	resp := postJSON(t, client, base+"/general/statuses", token, map[string]any{"name": "Review"})
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("create status: want 200, got %d", resp.StatusCode)
	}
...
func TestBoardStatusFilterHidesLanesAndCards(t *testing.T) {
	...
	body := getBody(t, client, base+"/general?status="+doing, http.StatusOK)
...
func TestBoardAssigneeFilterByUnassignedAndMe(t *testing.T) {
	...
	shown := getBody(t, client, base+"/general?assignee=unassigned", http.StatusOK)
	...
	hidden := getBody(t, client, base+"/general?assignee=me", http.StatusOK)
...
func TestMilestoneDiamondTintedByStatus(t *testing.T) {
	...
	postJSON(t, client, base+"/general/items/"+id+"/milestone", token, map[string]any{
		"is_milestone": true,
	}).Body.Close()
