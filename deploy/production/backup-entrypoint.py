...
exe = Path('/usr/local/bin/acta-server')
digest = hashlib.sha256(exe.read_bytes()).hexdigest()
release = Path('/releases')/digest
release.mkdir(parents=True, exist_ok=True)
if not (release/'acta-server').exists():
    shutil.copyfile(exe, release/'acta-server')
    os.chmod(release/'acta-server', 0o755)
...
for target in [*config.iterdir(), release/'acta-server']:
...
os.execvp('gosu', ['gosu', '999:10001', 'acta-backup', '--config', str(config/'backup.json'), *sys.argv[1:]])
